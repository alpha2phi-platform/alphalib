...
def get_gross_margin(income_df):
    current = float(income_df.loc["grossProfit"][0]) / float(
        income_df.loc["totalRevenue"][0]
    )
    previous = float(income_df.loc["grossProfit"][1]) / float(
        income_df.loc["totalRevenue"][1]
    )
    return current - previous
...
def calculate_piotroski_score(income_df, balance_df, cash_df) -> int:
    score = 0
